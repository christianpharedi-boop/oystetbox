...
class OysterBoxTests(unittest.TestCase):
    ...
    def test_pxd033169_three_round_audit_closes_unresolved_identity_graph(self):
        audit = (ROOT / "experiments/0.2/decisions/PXD033169_AUDIT.yaml").read_text(encoding="utf-8")
        identity = (ROOT / "experiments/0.2/acquisition/PXD033169_IDENTITY_GRAPH.yaml").read_text(encoding="utf-8")
        findings = (ROOT / "experiments/0.2/acquisition/PXD033169_ROUND2_ROUND3_FINDINGS.md").read_text(encoding="utf-8")
        self.assertIn("audit_status: CLOSED_SEARCH_BUDGET_EXHAUSTED", audit)
        self.assertIn("benchmark_v1_decision: EXCLUDE_PENDING_NEW_AUTHORITATIVE_EVIDENCE", audit)
        self.assertIn("run_to_sample: UNRESOLVED", audit)
        self.assertIn("sample_to_subject: UNRESOLVED", audit)
        self.assertIn("validation_subtype: UNRESOLVED", audit)
        self.assertIn("rounds_completed: 3", audit)
        self.assertIn("search_exhausted: true", audit)
        self.assertIn("mapping_status: UNRESOLVED_AFTER_THREE_ROUNDS", identity)
        self.assertIn("Round 3 status: no new authoritative", findings)
        self.assertIn("No additional archaeology is authorized", findings)

    def test_pxd033169_metadata_audit_preserves_identity_and_outcome_boundaries(self):
        audit = (ROOT / "experiments/0.2/decisions/PXD033169_AUDIT.yaml").read_text(encoding="utf-8")
        identity = (ROOT / "experiments/0.2/acquisition/PXD033169_IDENTITY_GRAPH.yaml").read_text(encoding="utf-8")
        discovery = (ROOT / "experiments/0.2/acquisition/PXD033169_DISCOVERY_MANIFEST.yaml").read_text(encoding="utf-8")
        validation = (ROOT / "experiments/0.2/acquisition/PXD033169_VALIDATION_MANIFEST.yaml").read_text(encoding="utf-8")
        sealed = (ROOT / "experiments/0.2/validation_outcomes/PXD033169_SEALED_OUTCOME_ARTIFACT.yaml").read_text(encoding="utf-8")
        protocol = (ROOT / "docs/DATASET_SCREENING_0.2.md").read_text(encoding="utf-8")
        self.assertIn("audit_status: CLOSED_SEARCH_BUDGET_EXHAUSTED", audit)
        self.assertIn("screening_decision: NEEDS_CLARIFICATION", audit)
        self.assertIn("modality: DIA", audit)
        self.assertIn("modality: TARGETED_MS", audit)
        self.assertIn("orthogonality:\n    status: NOT_ESTABLISHED", audit)
        self.assertIn("run_to_sample: UNRESOLVED", audit)
        self.assertIn("maximum_search_rounds: 3", audit)
        self.assertIn("search_exhausted: true", audit)
        self.assertIn("outcome_contents_exposed_to_scoring: false", sealed)
        self.assertIn("prediction_artifact_sha256_required_before_release: true", sealed)
        self.assertIn("contract: Run -> Sample -> Subject -> Cohort", identity)
        self.assertIn("sample_to_subject:\n    status: UNRESOLVED", identity)
        self.assertIn("information_cutoff: null", discovery)
        self.assertIn("admissible_to_scoring: false", validation)
        self.assertIn("independent", protocol)
        self.assertIn("three search rounds", protocol)
